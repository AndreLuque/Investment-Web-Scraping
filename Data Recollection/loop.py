...
for numero_enlace in range(len(lista_enlaces)):
    ...
    try:
        ...
        with open(lista_nombre_csv[numero_enlace], 'w', newline='\n') as csvfile:
            ...
            #el primer enlace no tiene datos sobre el volumen
            if numero_enlace == 0:
                csvtool.writerow(['Date', 'Price', 'Open', 'High', 'Low', '%Change'])
                for elem in tabla:
                    elem = elem.split()
                    """ elem[0] = mes, elem[1] = dia(segido de una coma), elem[2] = año,
                     elem[3] = precio inicial, elem[4] = precio apertura, elem[5] = maximo precio del dia, elem[6] = minimo precio del dia, elem[7] = variación diaria"""
                    csvtool.writerow([elem[0] + " " + elem[1] + " " + elem[2], elem[3], elem[4], elem[5], elem[6], elem[7]])
            else:
                csvtool.writerow(['Date', 'Price', 'Open', 'High', 'Low', 'Volume', '%Change'])    
                for elem in tabla:
                    elem = elem.split()
                    """ elem[0] = mes, elem[1] = dia(segido de una coma), elem[2] = año,
                     elem[3] = precio inicial, elem[4] = precio apertura, elem[5] = maximo precio del dia, elem[6] = minimo precio del dia, elem[7] = volumen,
                     elem[8] = variación diaria"""
                    csvtool.writerow([elem[0] + " " + elem[1] + " " + elem[2], elem[3], elem[4], elem[5], elem[6], elem[7], elem[8]])
    time.sleep(5)
driver.close()
